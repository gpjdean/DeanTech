...
K8s 终端、清单应用和部分工作负载操作会优先使用平台随包发布的 `kubectl`，固定目录如下：

```text
backend/tools/kubectl/linux-amd64/kubectl
backend/tools/kubectl/linux-arm64/kubectl
backend/tools/kubectl/darwin-arm64/kubectl
```

如果发布包内存在对应平台二进制，后端会优先使用它；只有未提供时才会回退到系统 `PATH`。
...
- 宣传、架构、Docker、K8s 与接口总文档见 [`docs/PLATFORM_PROMOTION_GUIDE.md`](docs/PLATFORM_PROMOTION_GUIDE.md)
